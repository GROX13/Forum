...
public class User {
	...
	public boolean addTheme(Theme theme) throws SQLException {
		...
		ResultSet resultSet = DBM.getDataFromDataBase(
		...
		resultSet = DBM.getDataFromDataBase(DataBaseInfo.MYSQL_TABLE_WARN,
				columns, values);
		if (resultSet.next())
			return false;

		themeManager.add(theme.getTitle(), theme.getDescription(),
				theme.getCreatorId(), theme.getCategoryId(), theme.getOpen());
		return true;
	}
	...
	public boolean addPost(Post post) throws SQLException {
		...
		if (resultSet.next()) {
			...
			ResultSet lastPostResultSet = DBM.getDataFromDataBase(
			...
			if ((currentPostDate.getTime() - lastPostDate.getTime())
					/ MILLISECONDS_IN_SECOND / SECONDS_IN_HOUR <= frequency)
				return false;
		}
	...
	public void sendMessage(Message message) {
		...
		messageManager.sendMessage(message.getMessageText(),
				message.getMessageImages(), message.getMessageVideos());
	}
	...
	protected boolean isBanned() throws SQLException {
